...
@pytest.mark.asyncio
async def test_checklist_feedback(app):
    import database
    cid = await database.save_checklist("005930", "2026-06-16", 85, 1, 1, 1, 1, 1, 1, "진입 검토")
    assert cid
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as c:
        r = await c.post("/api/checklist/outcome", json={"id": cid, "outcome": "WIN"})
        assert r.status_code == 200
        h = await c.get("/api/checklist/history")
        assert h.status_code == 200
        entry = [x for x in h.json() if x["id"] == cid][0]
        assert entry["outcome"] == "WIN"
        st = await c.get("/api/checklist/stats")
        assert st.status_code == 200
        band = [b for b in st.json()["bands"] if b["band"].startswith("진입검토")][0]
        assert band["win"] >= 1 and band["win_rate"] is not None
