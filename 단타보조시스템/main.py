...
async def run_checklist(body: CheckIn, request: Request):
    ...
    cid = await database.save_checklist(
...
# ── 체크리스트 결과 피드백/분석 ───────────────────────
@app.get("/api/checklist/history")
async def checklist_history(limit: int = 50):
    return await database.get_checklist_history(limit)


class OutcomeIn(BaseModel):
    id: int
    outcome: str  # WIN / LOSS / NONE

@app.post("/api/checklist/outcome")
async def checklist_outcome(body: OutcomeIn):
    oc = body.outcome if body.outcome in ("WIN", "LOSS") else None
    await database.set_checklist_outcome(body.id, oc)
    return {"ok": True}


@app.get("/api/checklist/stats")
async def checklist_stats():
    return await database.get_checklist_stats()
